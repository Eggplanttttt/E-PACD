    ...
    <ul class="navbar-nav ms-auto flex-row align-items-center gap-2">
      ...
      <li class="nav-item dropdown order-0 order-sm-1">
        ...
        <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="userDropdown" id="accountDropdownMenu">
          ...
          {{-- Settings --}}
          <li>
            <a class="dropdown-item" href="{{ route('alumni.settings') }}">
  ...
  <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
    <div class="modal-content vd-complaint-modal">
      ...
      <div class="modal-body">
        ...
        {{-- COMPLAINT FORM --}}
        <form id="complaintForm" class="vd-form" action="{{ route('alumni.complaints.store') }}" method="POST" enctype="multipart/form-data">
          @csrf
...
<div class="dashboard-container">
    <header class="client-header text-center mb-4">
        ...
        <p>You're now connected to E-PACD Chat Support (Alumni)</p>
    </header>
    ...
    <div class="mobile-faq-toggle d-lg-none mb-3">
        ...
        <div class="collapse mt-2" id="mobileFaq">
            <div class="faq-sidebar">
                ...
                <li data-question="How do I get my alumni clearance signed after graduation?">How do I get my alumni clearance signed after graduation?</li>
                <li data-question="Can someone else process my clearance on my behalf? ">Can someone else process my clearance on my behalf? </li>
                <li data-question="How do I update my alumni records after graduation? ">How do I update my alumni records after graduation? </li>
                <li data-question="Is alumni data updating required? ">Is alumni data updating required? </li>
                <li data-question="What are the benefits of being part of the Alumni Association? ">What are the benefits of being part of the Alumni Association? </li>
                <li data-question="How do alumni get involved in university projects? ">How do alumni get involved in university projects? </li>
                <li data-question="Does the Alumni Office conduct alumni homecoming events? ">Does the Alumni Office conduct alumni homecoming events? </li>
                </ul>
            </div>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function appendMessage(sender, message, id = null, messageObj = {}) {
    ...
    if (sender.toLowerCase().startsWith('alumni/')) {
  ...
  try {
    ...
    const res = await fetch('/alumni/messages');
    ...
    data.messages.forEach(msg => {
      ...
      if (!document.getElementById(id)) {
        ...
        if (msg.sender.toLowerCase().startsWith('alumni/')) {
          msgDiv.classList.add('message-user');
          msgDiv.innerHTML = `<strong>You:</strong> ${msg.message || ''}${formatAttachment(msg)}`;
        } else {
          msgDiv.classList.add('message-system');
          msgDiv.innerHTML = `<strong>${msg.sender}:</strong> ${msg.message || ''}${formatAttachment(msg)}`;
        }
    ...
    if (newMessageAdded && !isAnyVideoPlaying()) {
      scrollToBottom(true);
    }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function refreshUnreadCount(){
    try{
      const res = await fetch("{{ route('alumni.notifications') }}", {
  ...
  markReadBtn?.addEventListener('click', async (e) => {
    ...
    await fetch("{{ route('alumni.notifications.markRead') }}", {
      method: "POST",
      headers: {
  ...
  deleteReadBtn?.addEventListener('click', async (e) => {
    ...
    await fetch("{{ route('alumni.notifications.deleteRead') }}", {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
        "X-CSRF-TOKEN": "{{ csrf_token() }}",
