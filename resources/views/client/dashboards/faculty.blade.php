    ...
    <ul class="navbar-nav ms-auto flex-row align-items-center gap-2">
      ...
      <li class="nav-item dropdown order-0 order-sm-1">
        ...
        <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="userDropdown" id="accountDropdownMenu">
          ...
          {{-- Settings --}}
          <li>
            <a class="dropdown-item" href="{{ route('faculty.settings') }}">
  ...
  <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
    <div class="modal-content vd-complaint-modal">
      ...
      <div class="modal-body">
        ...
        {{-- COMPLAINT FORM --}}
        <form id="complaintForm" class="vd-form" action="{{ route('faculty.complaints.store') }}" method="POST" enctype="multipart/form-data">
          @csrf
...
<div class="dashboard-container">
    ...
    <header class="client-header text-center mb-4">
        ...
        <p>You're now connected to E-PACD Chat Support (Faculty)</p>
    </header>
    ...
    <div class="mobile-faq-toggle d-lg-none mb-3">
        ...
        <div class="collapse mt-2" id="mobileFaq">
            <div class="faq-sidebar">
                ...
                <li data-question="How do I access class lists? ">How do I access class lists? </li>
                <li data-question="What should I do if I need to correct a grade already submitted? ">What should I do if I need to correct a grade already submitted? </li>
                <li data-question="How do I request a Certificate of Employment or Service Record? ">How do I request a Certificate of Employment or Service Record? </li>
                <li data-question="How do I access the faculty portal for encoding grades? ">How do I access the faculty portal for encoding grades? </li>
                <li data-question="How do I request the use of a university vehicle? ">How do I request the use of a university vehicle? </li>
                </ul>
            </div>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function scrollToBottom(force = false) {
        ...
        if (force || isNearBottom) {
            // run multiple passes to ensure true bottom
  ...
  // Timer for 30-second auto system message
  let systemReplyTimer = null;
  ...
  function appendMessage(msg) {
    ...
    const attachmentHtml = formatAttachment(msg);

    if (msg.sender.toLowerCase().startsWith('faculty/')) {
      msgDiv.classList.add('message-user');
      msgDiv.innerHTML = `<strong>You:</strong> ${msg.message || ''}${attachmentHtml}`;
    } else {
      msgDiv.classList.add('message-system');
      msgDiv.innerHTML = `<strong>${msg.sender}:</strong> ${msg.message || ''}${attachmentHtml}`;
    }
  ...
  async function fetchMessages() {
    try {
      ...
      const res = await fetch('/faculty/messages');
  ...
  async function sendMessage(message, fromFaq = 0) {
    ...
    try {
      ...
      if (!displayedMessageIds.has(result.message.id)) {
        appendMessage(result.message);
        displayedMessageIds.add(result.message.id);
      }

      if (result.system_message && !displayedMessageIds.has(result.system_message.id)) {
        appendMessage(result.system_message);
        displayedMessageIds.add(result.system_message.id);
      }
      ...
      // Reset the 30-second system reply timer
      resetSystemReplyTimer();
    } catch (err) {
  ...
  function resetSystemReplyTimer() {
    if (systemReplyTimer) clearTimeout(systemReplyTimer);
    systemReplyTimer = setTimeout(checkSystemReply, 30000); // 30 seconds
  }

  async function checkSystemReply() {
    try {
      const res = await fetch('/faculty/messages');
      const data = await res.json();
      ...
      // Filter for any new System messages that haven't been displayed yet
      const newSystemMsgs = data.messages.filter(msg => 
        msg.sender === 'System' && !displayedMessageIds.has(msg.id)
      );
      ...
      // Restart the 30-second timer for next inactivity
      resetSystemReplyTimer();
    } catch (err) {
      console.error("System reply check error:", err);
    }
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function refreshUnreadCount(){
    try{
      const res = await fetch("{{ route('faculty.notifications') }}", {
  ...
  markReadBtn?.addEventListener('click', async (e) => {
    ...
    await fetch("{{ route('faculty.notifications.markRead') }}", {
      method: "POST",
      headers: {
  ...
  deleteReadBtn?.addEventListener('click', async (e) => {
    ...
    await fetch("{{ route('faculty.notifications.deleteRead') }}", {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
        "X-CSRF-TOKEN": "{{ csrf_token() }}",
