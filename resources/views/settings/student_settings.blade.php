...
@section('title', 'Student Account Settings')
...
<div class="container mt-4" style="max-width: 500px;">
    @php
        $dashboardRoute = route('client.dashboard.student'); // student-only dashboard
    @endphp
    ...
    <form id="password-form" method="POST" action="{{ route('student.settings.updatePassword') }}">
        @csrf
        @method('PUT')
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.addEventListener('popstate', () => {
        window.location.href = "{{ route('client.dashboard.student') }}";
    });
});
